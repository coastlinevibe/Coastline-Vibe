...
export default function EditProfilePage() {
  ...
  const [communityId, setCommunityId] = useState<string>('miami'); // Default community
  ...
  const [activeTab, setActiveTab] = useState('profile'); // 'profile' or 'location'
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-100 to-cyan-100 flex flex-col items-center py-10">
      <div className="w-full max-w-lg bg-white/90 rounded-2xl shadow-xl border border-cyan-100 p-8 flex flex-col gap-8">
        ...
        <div className="flex border-b border-gray-200">
          <button
            className={`px-4 py-2 font-medium ${activeTab === 'profile' ? 'text-cyan-600 border-b-2 border-cyan-500' : 'text-gray-500 hover:text-gray-700'}`}
            onClick={() => setActiveTab('profile')}
          >
            Profile Info
          </button>
          <button
            className={`px-4 py-2 font-medium ${activeTab === 'location' ? 'text-cyan-600 border-b-2 border-cyan-500' : 'text-gray-500 hover:text-gray-700'}`}
        ...
        {activeTab === 'profile' ? (
          ...
          // LocationSettings tab
          profileId && <LocationSettings userId={profileId} communityId={communityId} />
        )}
      </div>
    </div>
  );
