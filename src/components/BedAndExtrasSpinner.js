...
const BedAndExtrasSpinner = ({ onAddItem, onRemoveItem }) => {
  ...
  const bedIcons = [
    { key: 'single_bed', iconSet: 'MaterialCommunityIcons', name: 'bed-single', color: '#333' },
    { key: 'double_bed', iconSet: 'MaterialCommunityIcons', name: 'bed-double', color: '#333' },
    { key: 'queen_bed', iconSet: 'MaterialCommunityIcons', name: 'bed-queen', color: '#333' },
    { key: 'bunk_bed', iconSet: 'MaterialCommunityIcons', name: 'bunk-bed', color: '#333' }
  ];
  
  const extrasIcons = [
    { key: 'pillows', iconSet: 'FontAwesome5', name: 'bed', color: '#333' },
    { key: 'crib', iconSet: 'FontAwesome5', name: 'baby-carriage', color: '#333' },
    { key: 'air_purifier', iconSet: 'MaterialCommunityIcons', name: 'air-purifier', color: '#333' }
  ];
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: () => {
        ...
        if (hovering && draggingIconKey) {
          ...
          if (!selectedItems.includes(draggingIconKey)) {
            const newSelectedItems = [...selectedItems, draggingIconKey];
            setSelectedItems(newSelectedItems);
            onAddItem && onAddItem(draggingIconKey);
          }
        }
  ...
  const removeItem = (itemKey) => {
    const newSelectedItems = selectedItems.filter(key => key !== itemKey);
    setSelectedItems(newSelectedItems);
    onRemoveItem && onRemoveItem(itemKey);
  };
  ...
  const findIconByKey = (key) => {
    const allIcons = [...bedIcons, ...extrasIcons];
  ...
  const renderSelectedIcon = (itemKey) => {
    const icon = findIconByKey(itemKey);
    if (!icon) return null;
    
    const IconComponent = icon.iconSet === 'MaterialCommunityIcons' ? MaterialCommunityIcons : FontAwesome5;
    
    return (
      <View key={itemKey} style={{ position: 'relative' }}>
      ...
      >
        {selectedItems.length === 0 ? (
          ...
          <View style={styles.selectedRow}>
            {selectedItems.map(itemKey => renderSelectedIcon(itemKey))}
          </View>
        )}
      </View>
      
      {/* Right spinner - Extras */}
      <View style={styles.spinnerColumn}>
        <FlatList
          data={extrasIcons}
          renderItem={renderIcon}
...
export default BedAndExtrasSpinner; 