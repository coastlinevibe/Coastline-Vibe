...
const UserTooltipDisplay: React.FC<UserTooltipDisplayProps> = ({ profile }) => {
    ...
    <div className="absolute left-0 top-full mt-2 w-72 md:w-80 bg-offWhite border border-grayLight rounded-xl shadow-elevated p-4 z-50">
      <div className="flex flex-col items-center mb-3">
        <Image
          ...
          width={80}
          height={80}
          className="rounded-full border-2 border-primaryTeal mb-2"
        />
        <div className="flex items-center">
          ...
          {profile.is_location_verified && (
            <span title="Verified Resident">
              <ShieldCheck size={16} className="ml-1 text-primaryTeal" />
            </span>
          )}
        </div>
        {profile.role && (
      ...
      {profile.email && (
        <div className="flex items-center text-sm text-darkCharcoal mb-1.5">
          <Mail size={14} className="mr-2 text-primaryTeal" />
      ...
      {profile.created_at && (
        <div className="text-sm text-darkCharcoal flex items-center pt-2 border-t border-sand">
          <CalendarDays size={14} className="mr-2 text-primaryTeal" />
          <span>
      ...
      {/* Placeholder for future actions like 'View Profile' button */}
      {/* 
      <div className="mt-3 pt-3 border-t border-sand">
        <Link href={`/profile/${profile.id}`} className="block w-full">
          <button className="w-full bg-primaryTeal hover:bg-seafoam text-offWhite font-semibold py-2 px-3 rounded-md text-sm transition duration-150 ease-in-out">
            View Profile
          </button>
        </Link>
      </div>
