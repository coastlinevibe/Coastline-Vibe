...
export default function PollPostForm({ onSubmit }: { onSubmit: (values: PollPostFormValues) => Promise<void>; }) {
  ...
  return (
    <form className="mb-4" onSubmit={handleSubmit(submitHandler)}>
      ...
      <input {...register('question', { required: true })} type="text" className="w-full border border-slate-300 rounded px-3 py-2 mb-2" placeholder="What's on your mind…?" disabled={isSubmitting} />
